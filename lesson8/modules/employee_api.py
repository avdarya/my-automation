...
class EmployeeApi:
  ...
  def get_employees_by_company(self, company_id: int) -> Response:
    return requests.get(f'{self.base_url}/employee', params={'company': company_id})
  
  def create_employee(self, headers: dict, body: dict) -> Response:
    return requests.post(f'{self.base_url}/employee', headers=headers, json=body)
  
  def get_employee_by_id(self, employee_id: int) -> Response:
    return requests.get(f'{self.base_url}/employee/{employee_id}')
  
  def edit_employee(self, headers: dict, id: int, body: dict) -> Response:
